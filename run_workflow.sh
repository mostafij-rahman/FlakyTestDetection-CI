#!/bin/bash
#bash run_workflow.sh project_sha_list.csv 100

if [[ $1 == "" ]] || [[ $2 == "" ]]; then
    echo "arg1 - Path to CSV file with project, SHA"
    echo "arg2 - Number of workflow runs"
    exit
fi

PROJECT_SHA_LIST_FILE=$1
WORKFLOW="ci-unit-broker-broker-gp2_modified-pom"
START=1
END=$2
...
for line in $(cat ${PROJECT_SHA_LIST_FILE}); do
	...
	NEW_WORKFLOW_FILE=${WORKFLOW}_${SHA}.yaml
