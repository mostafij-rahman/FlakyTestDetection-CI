#!/bin/bash
#bash run_local_broker_client_impl.sh project_sha_list.csv 1
...
START=1
END=$2
WORKFLOW=unit-broker-client-impl
...
for line in $(cat ${PROJECT_SHA_LIST_FILE}); do
	...
	for ((i = $START ; i <= $END ; i++)); do
	  ...
	  #building modules
	  mvn -B -ntp -q install -Pcore-modules,-main -DskipTests |& tee ${LOCAL_LOG_DIR}/local_run_log_${i}.log
	  
	  #run unit test BROKER_FLAKY
	  build/run_unit_group.sh BROKER_CLIENT_IMPL |& tee ${LOCAL_LOG_DIR}/local_run_log_${i}.log
