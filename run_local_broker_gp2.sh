#!/bin/bash
#bash run_local_broker_gp2.sh project_sha_list.csv 1
...
for line in $(cat ${PROJECT_SHA_LIST_FILE}); do
	...
	for ((i = $START ; i <= $END ; i++)); do
	  ...
	  build/run_unit_group.sh BROKER_GROUP_2 |& tee ${LOCAL_LOG_DIR}/local_run_log_${i}.log
