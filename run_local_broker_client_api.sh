#!/bin/bash
#bash run_local_broker_client_api.sh project_sha_list.csv 1
...
START=6
END=$2
WORKFLOW=unit-broker-client-api
...
for line in $(cat ${PROJECT_SHA_LIST_FILE}); do
	...
	for ((i = $START ; i <= $END ; i++)); do
	  ...
	  build/run_unit_group.sh BROKER_CLIENT_API |& tee ${LOCAL_LOG_DIR}/local_run_log_${i}.log
